...
- **`simulate`:** we *simulate* your intent with and without a prefund and prefund the exact SOL it consumes — this also covers non-ATA SOL costs (bridge native fees) automatically and never over-prefunds.
...
| `GASLESS_PREFUND_SIZING` | `scan` | How the Solana user-SOL prefund is sized: `scan` (static ATA-create scan), `simulate` (measure the exact SOL via a with/without-prefund simulation on every path — fixes 0-SOL swap-fee estimates and catches non-ATA native fees), or `shadow` (scan + log the sim delta, no behaviour change). See [Solana architecture › Prefund sizing](./solana-architecture.md#prefund-sizing). |
