...
Any EIP-7702-capable EVM chain can be added by deploying `GaslessDelegate` and listing the chain in `backend/config.yaml`. BSC, Base and Arbitrum are live today.

The user delegates their EOA to the `GaslessDelegate` Solidity contract via EIP-7702, signs an EIP-712 batch describing what they want to do, and the operator submits the type-4 transaction and pays the gas. The user pays the operator in the token of their choice.
